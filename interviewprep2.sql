...
from s left join r on s.sid = r.sid 
where r.bid IN (select r.bid from s left join r on s.sid = r.sid having count(distinct(s.sid)) > 5) group by age; 

SELECT ROUND(235.4156666666, 2) AS RoundValue;
